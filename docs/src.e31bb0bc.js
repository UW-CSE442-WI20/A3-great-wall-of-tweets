...
drawScatter(myList); // Set the dimensions and margins of the graph
...
d3.select("#link2").on("click", function (d) {
  d3.event.preventDefault();
  d3.csv(wordData).then(function (data) {
    var clintonResults = [];
    data.forEach(function (d) {
...
d3.select("#link3").on("click", function (d) {
  d3.event.preventDefault();
  d3.csv(wordData).then(function (data) {
    var americaResults = [];
    data.forEach(function (d) {
...
d3.select("#form").on("submit", function (d) {
  ...
  if (valid) {
    ...
    for (var _i3 = 0; _i3 < searchResults.length; _i3++) {
      searchResults[_i3] = parseTime(searchResults[_i3]);
    }
  }
...
function drawScatter(searchResults) {
  d3.csv(csvFile).then(function (data) {
    ...
    .translateExtent([[0, 0], [width, height]]).extent([[0, 0], [width, height]]).on("zoom", zoomed); //svg.call(zoom)
    // Add X axis
    ...
    scatter.selectAll("dot").data(data).enter().append("circle").attr("cx", function (d) {
      ...
      div.text(d.Tweet_Text).style("left", d3.event.pageX + "px").style("top", d3.event.pageY - 28 + "px");
    }).on("mouseout", function (d) {
      div.transition().duration(500).style("opacity", 0);
    });
    var scat = scatter.selectAll("circle"); // Update chart when zooming
...
},{"d3":"../node_modules/d3/index.js","./cleaner_csv.csv":"cleaner_csv.csv","./words_to_id.csv":"words_to_id.csv"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
